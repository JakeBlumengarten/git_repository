...
ggplot(chicago_bears_24, aes(x = week)) +
  ...
  geom_line(aes(y = winperc * max(yards_per_week)), color = "darkgreen", linetype = "dashed", size = 1) + 
  scale_y_continuous(
    name = "Total Yards Gained",
    sec.axis = sec_axis(~./max(chicago_bears_24$yards_per_week), name = "Win Percentage")
  ) +
  labs(title = "Chicago Bears Offensive Yards & Win Percentage (2024 Season)",
       x = "Week") +
  theme_minimal() +
  theme(
  ...
  annotate("text", x = 13, y = 250, label = "Eberflus Fired", color = "red", size = 5, fontface = "bold", angle = 75)
ggsave("chicago_yards_by_week_R.png", path = "/Users/jakeblumengarten/git_repository/nfl_analysis/plots/")
```
...
ggsave("teams_ypg.png", path = "/Users/jakeblumengarten/git_repository/nfl_analysis/plots/", plot = teamsypg, width = 10, height = 10, dpi = 300)
```
## Correlation Between Average Yards and Winning Percentage
```{r}
...
ggsave("ypg_winpct_corr.png", path = "/Users/jakeblumengarten/git_repository/nfl_analysis/plots/", width = 10, height = 10, dpi = 300)
```


## Looking at QB Influence on Win Percentage
```{r}
qbs <- season_24 %>%
...
ggsave("averageydsqb.png", path = "/Users/jakeblumengarten/git_repository/nfl_analysis/plots/", width = 10, height = 10, dpi = 300)
```